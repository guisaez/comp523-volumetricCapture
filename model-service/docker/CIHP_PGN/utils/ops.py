...
def conv2d(input_, output, kernel, stride, relu, bn, name, stddev=0.01):
    ...
    # Convolution for a given input and kernel
        ...
        w = tf.compat.v1.get_variable('w', shape, initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev), use_resource=False)
        conv = tf.nn.conv2d(input=input_, filters=w, strides=[1, stride, stride, 1], padding='SAME')
        # Add the biases
        b = tf.compat.v1.get_variable('b', [output], initializer=tf.compat.v1.constant_initializer(0.0), use_resource=False)
...
def linear(input_, output, name, stddev=0.02, bias_start=0.0):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        matrix = tf.compat.v1.get_variable("Matrix", [shape[1], output], tf.float32,
                                 tf.compat.v1.random_normal_initializer(stddev=stddev), use_resource=False)
        bias = tf.compat.v1.get_variable("bias", [output], initializer=tf.compat.v1.constant_initializer(bias_start), use_resource=False)
...
def gcn(input_, output, kernel, stride, relu, bn, name, stddev=0.01):
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        w1_1 = tf.compat.v1.get_variable('w1_1', left_shape_k_1, initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev), use_resource=False)
        w1_2 = tf.compat.v1.get_variable('w1_2', left_shape_1_k, initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev), use_resource=False)
        w2_1 = tf.compat.v1.get_variable('w2_1', right_shape_1_k, initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev), use_resource=False)
        w2_2 = tf.compat.v1.get_variable('w2_2', right_shape_k_1, initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev), use_resource=False)
        b1_1 = tf.compat.v1.get_variable('b1_1', [output], initializer=tf.compat.v1.constant_initializer(0.0), use_resource=False)
        b1_2 = tf.compat.v1.get_variable('b1_2', [output], initializer=tf.compat.v1.constant_initializer(0.0), use_resource=False)
        b2_1 = tf.compat.v1.get_variable('b2_1', [output], initializer=tf.compat.v1.constant_initializer(0.0), use_resource=False)
        b2_2 = tf.compat.v1.get_variable('b2_2', [output], initializer=tf.compat.v1.constant_initializer(0.0), use_resource=False)

        conv1_1 = tf.nn.conv2d(input=input_, filters=w1_1, strides=[1, stride, stride, 1], padding='SAME')
        ...
        conv1_2 = tf.nn.conv2d(input=conv1_1, filters=w1_2, strides=[1, stride, stride, 1], padding='SAME')
        ...
        conv2_1 = tf.nn.conv2d(input=input_, filters=w2_1, strides=[1, stride, stride, 1], padding='SAME')
        ...
        conv2_2 = tf.nn.conv2d(input=conv2_1, filters=w2_2, strides=[1, stride, stride, 1], padding='SAME')
