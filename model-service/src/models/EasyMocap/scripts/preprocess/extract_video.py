...
def mkdir(x): return os.makedirs(x, exist_ok=True)
...
def extract_2d(openpose, image, keypoints, render, args):
    ...
    if not skip:
        ...
        if args.highres != 1:
            cmd = cmd + \
                ' --net_resolution -1x{}'.format(
                    int(16*((368*args.highres)//16)))
        if args.handface:
            cmd = cmd + ' --hand --face'
        if args.render:
            if os.path.exists(join(os.getcwd(), render)):
                cmd = cmd + \
                    ' --write_images {}'.format(join(os.getcwd(), render))
            else:
...
def load_openpose(opname):
    mapname = {'face_keypoints_2d': 'face2d',
               'hand_left_keypoints_2d': 'handl2d', 'hand_right_keypoints_2d': 'handr2d'}
...
def convert_from_openpose(path_orig, src, dst, annotdir):
    ...
    for inp in tqdm(inputlist, desc='{:10s}'.format(os.path.basename(dst))):
        ...
        imgname = annotname.replace(
            annotdir, 'images').replace('.json', '.jpg')
...
def extract_yolo_hrnet(image_root, annot_root, ext='jpg', use_low=False):
    ...
    for nf, imgname in enumerate(tqdm(imgnames)):
        annotname = join(annot_root, os.path.basename(
            imgname).replace('.{}'.format(ext), '.json'))
        annot = create_annot_file(annotname, imgname)
        img0 = cv2.imread(imgname)
        annot['annots'] = detect_frame(detector, img0, 0)
        for i in range(len(annot['annots'])):
            x = annot['annots'][i]
            x['area'] = max(x['bbox'][2] - x['bbox'][0],
                            x['bbox'][3] - x['bbox'][1])**2
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='openpose', choices=[
                        'openpose', 'yolo-hrnet'], help="model to extract joints from image")
    parser.add_argument('--ext', type=str, default='jpg',
                        choices=['jpg', 'png'], help="image file extension")
        ...
        if not args.no2d:
            for sub in subs:
                ...
                if mode == 'openpose':
                    ...
                    extract_yolo_hrnet(
                        image_root, annot_root, args.ext, args.low)
    else:
